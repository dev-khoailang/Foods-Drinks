...
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header"><?=lang('edit_admin_group');?></h1>
    </div>
    <!-- /.col-lg-12 -->
</div>
<!-- /.row -->
    ...
    <div class="col-lg-12">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <div class="row">
                 <form role="form" action="<?=base_url();?>cms/admin/editGroup/<?=$group->id;?>" method="post" enctype="multipart/form-data">
                    <div class="col-lg-12" style="padding-left: 200px;">
                        ...
                        <div class="form-group input-group col-xs-8">
                         ...
                         <div class="form-group input-group col-xs-8">
                            ...
                            <textarea class="form-control" rows="3" name="desc"><?=$group->desc;?></textarea>
                        </div>
...
foreach ($controllers as $cont => $methods): ?>
                                        <tr class="odd">
                                            ...
                                            <div class="form-group">
                                                <div class="checkbox">
                                                    <label>
                                                        <input name="cont_permission[]" type="checkbox" value="<?=$cont;?>" class="check_<?=$cont;?>" <?=in_array($cont, $group->permissions) ? ' checked' : '';?>>
...
foreach ($methods as $method): ?>
                                                <div class="form-group <?='cont_' . $cont;?>" style="<?=(in_array($cont . '/' . $method, $group->permissions) or in_array($cont, $group->permissions)) ? '' : ' display:none';?>">
                                                <div class="checkbox">
                                                    <label>
                                                        <input name="method_permission[<?=$cont;?>][]" type="checkbox" value="<?=$cont . '/' . $method;?>" class="method_<?=$cont;?>" onchange="methodChecked('<?=$cont;?>')" <?=(in_array($cont . '/' . $method, $group->permissions) or in_array($cont, $group->permissions)) ? ' checked' : '';?>>
                                                        &nbsp;&nbsp;<?=ucwords($method);?>
